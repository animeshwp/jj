...
class Mod_category extends CI_Model {        
    ...
    function save($data){
        ...
        $result = $this->db->insert($this->_jj_category,$data);
        if ($result > 0){
            return $result;
        }
    }    
    ...
    function save_subcategory($data){
        ...
        // print_r($data); exit;
        $result = $this->db->insert($this->_jj_subcategory,$data);
        if ($result > 0){
            return $result;
        }
    }
    ...
    public function subcategories(){
        return $this->db->get($this->_jj_subcategory, array('status' => 1))->result();
    }

    public function view_category(){
        return $this->db->get($this->_jj_category)->result();
    }

    public function view_sub_category(){
        return $this->db->get($this->_jj_subcategory)->result();
    }


    function edit_category($id){
        return $this->db->get_where($this->_jj_category, array('id' => $id) )->row();                            
    
    }

    function edit_subcategory($id){
        return $this->db->get_where($this->_jj_subcategory, array('id' => $id) )->row();                            
  
    }

    function update_category($id, $data){
 
        $result = $this->db->update($this->_jj_category, $data, array('id' => $id ));
        if ($result > 0){
            return $result;
        }
    }
    ...
    function update_subcategory($id, $data){
        ...
        $result = $this->db->update($this->_jj_subcategory, $data, array('id' => $id ));
        if ($result > 0){
            return $result;
        }
    }
...
//        ECHO $result; EXIT;
    ...
    public function categories(){
        ...
        return $this->db->get_where($this->_jj_category, array('catStatus' => 1) )->result();
    } 
    ...
    public function category_name_acc_to_id($id){
        $this->db->select('title');
        return $this->db->get_where($this->_jj_category, array('id' => $id) )->row();
    }
