...
class Mod_slide extends CI_Model {
    ...
    function insert_slide($img_info) {
...
//        print_r ($slide_info); exit();
    ...
    function view_inactiveslide() { // model for home page slide
    ...
    function view_hslide() { // model for home page slide
        
        $slide_query = $this->db->get_where('vimg_slide', array('slide_status' => 1));
        return $slide_query->result();
...
//        print_r ($slide_result); exit();              
    ...
    function update_slide($nemu_id, $img_info) {
        $day = date("Y-m-d H:i:s");
        if (empty($img_info)) {
            $slide_info = array(
                "slide_name" => $this->input->post('slidename'),
