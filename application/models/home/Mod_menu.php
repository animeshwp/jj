...
//        $this->db->order_by('menu_order', 'ASC');
    ...
    function view_hmenu(){ // model for home page menu
        $this->db->order_by('menu_order ASC');
        $menu_query = $this->db->get_where('vimg_menu', array('menu_status' => 1, 'menu_loc' => 1));       
        $menu_result = $menu_query->result_array();           
        return $menu_result;
    }


    function view_hmenu_left(){ // model for home page menu
        $this->db->order_by('menu_order ASC');
        $menu_query = $this->db->get_where('vimg_menu', array('menu_status' => 1, 'menu_loc' =>2));       
        $menu_result = $menu_query->result_array();           
        return $menu_result;
    }
...
//        print_r ($menu_result); exit();              
    ...
    function detail_menu($nemudetail_id){
        ...
        $menudetail_result = $menudetail_query->row_array();               
//        echo "<pre>";
